        ...
        while (true) {
            ...
            if (respostaMenu == 2) {
                ...
                while (true) {
                    ...
                    System.out.println("\nAonde poderá buscar o pet com os seguintes criterios apenas:\n" +
                            "1. Nome ou sobrenome\n" + "2. Sexo\n" + "3. Idade\n" + "4. Peso\n" + "5. Raça\n" + "6. Endereço");
                ...
                while (true) {
                    System.out.print("\nDeseja utilizar a data de cadastro, para encontrar o seu Pet (Sim/Nao)? ");
                    respotaData = input.nextLine();

                    if (!respotaData.equalsIgnoreCase("Sim") && !respotaData.equalsIgnoreCase("Nao")) {
                        System.out.println("O usuario entrou com uma resposta errada, Tente novamente! ");
                        continue;
                    }
                    break;
                }

                if (respotaData.equalsIgnoreCase("Sim")) {
                    while (true) {
                        System.out.print("Entre com a data de cadastro do seu Pet: ");
                        String entradaDataUsuario = input.nextLine();

                        DateTimeFormatter formatoBR = DateTimeFormatter.ofPattern("dd/MM/yyyy");
                        DateTimeFormatter formatoArquivo = DateTimeFormatter.ofPattern("yyyyMMdd");

                        try {
                            LocalDate objetoData = LocalDate.parse(entradaDataUsuario, formatoBR);
                            dataCadastro = objetoData.format(formatoArquivo);
                        } catch (DateTimeParseException e) {
                            System.out.println("\nErro: Voce precisa digitar uma data valida no formato (dd/MM/yyyy)");
                            continue;
                        }
                        break;
                    }
                    ...
                    for (File arquivo : arquivos) {
                        if (arquivo.getName().contains(dataCadastro)) {
                            hasOneDate = true;
                        }
                    }

                    if (!hasOneDate) {
                        System.out.println("\nNao foi encontrado pet cadastrado com a data passada Pelo Usuario! ");
                        System.out.println("Seguimos com a busca por criterios, para encontrar seu Pet! ");
                    }
                } else {
                    dataCadastro = "-";
                }
                ...
                if (respostaCriterio.equalsIgnoreCase("Sim")) {
                    ...
                    while (true) {
                        ...
                        if (Pet.temAtributo(resposta1) == false || Pet.temAtributo(resposta2) == false) {
                            System.out.println("\nAlgum Criterio foi passado de forma invalida! Tente Novamente: ");
                            continue;
                        } else {
                    ...
                    CadastrosPet.find2Criterion(tipoPet, criterio1, criterio2, dataCadastro);
                }
                ...
                if (respostaCriterio.equalsIgnoreCase("Nao")) {
                    ...
                    while (true) {
                        ...
                        if (Pet.temAtributo(resposta) == false) {
                            System.out.println("\nCriterio foi passado de forma invalida! Tente Novamente: ");
                            continue;
                        } else {
                    ...
                    CadastrosPet.findCriterion(tipoPet, criterio, dataCadastro);
                }

                //dentro do 2
            }
            //dentro do loop do menu
        }
        //dentro do main
    }
}
